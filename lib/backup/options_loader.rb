module Backup23
  module OptionsLoader
    ...
    def load_options
      ...
      Log.exit "Can not find #{options_file_name} file" unless File.exists?(options_file_path)
      options_hash = {}
      ...
      if options_hash.nil? || !options_hash.is_a?(Hash) || options_hash.keys.empty?
        Log.exit "Options file is empty #{options_file_path}"
      end
...
private
    ...
    def load_default_options(options_hash)
      ...
      if defaults_hash.nil? || !defaults_hash.is_a?(Hash) || defaults_hash.empty?
        Log.exit "Missing 'default_options' configuration section"
      end
    ...
    def load_task_options(options_hash)
      ...
      if tasks_array.nil? || !tasks_array.is_a?(Array) || tasks_array.empty?
        Log.exit "There are no backup tasks in configuration file" 
      end
      @task_options = [] 
